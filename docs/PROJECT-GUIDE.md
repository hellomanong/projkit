...
| `docs/review/` | 设计文档（`/spec-dev-doc` 的产出）：把该模块已定稿的 SPEC 投影成**自包含 HTML**，开篇「整体设计」给整体架构图与整体流程图，其后各章取「这章讲什么 / 图 / 为什么这么定 / 要你拍的 / 缺口」五栏，实现细节留在 SPEC；**一个模块一份，SPEC 改了就覆盖它**，要留不再变的快照才显式指定新版本号 | 模块 SPEC 定稿后，需要一份给人拍板、也能对外发的稿子时生成；反馈中的新信息由你消化回流 SPEC 后重生成 |
...
模块 SPEC 定稿后，随时 `/spec-dev-doc <模块> [版本号]`——把 SPEC 投影成一份自包含 HTML 设计文档落 `docs/review/`（版本化快照，旧版不覆盖）。它**只读 SPEC 一份输入**，只取前五栏（含图），「怎么做」一个字不取；章节镜像 SPEC 的章；全文只有一个端到端例子、文字 ≤6 屏（图不计）。因为只有一个输入，它没法二次创作——实现细节不进来，也就不需要靠折叠压缩。它是三环的**旁路**：只读 SPEC、不产生三环的输入；看稿反馈里的实质新信息由你消化，需求级走 `/spec-interview` 增量轮、设计级走 `/spec-design` 修订轮，回流后重生成。
