...
**注意这是正式文件**：不含访谈纪要、待定问题等过程内容——那些住草稿里，定稿即删。**注意「方案设计」节的图**：**稳态全貌 + 本轮标记合一张**——全部子组件连同并存的旧路径（轮询）都画，本轮新建带 `*`、图下有图例、箭头带方向；详细程度对标全局图、主要取材该模块解读（`docs/prd/解读/<模块>.md`），解读里的每条主要流程都要能在图上走通（本例模块小所以图小，真实模块图通常更满，但节点仍控制在十来个）；项目整体架构图不放 SPEC 里，住 `docs/ARCHITECTURE.md`（两级分工见 `reference.md` §二）。**注意第 4、5 步**：每步都写了**预期结果**，「跑测试」不算验证步骤。**注意「不在范围内」最后一条**：PRD 未定的部分显式圈出来留给下一轮，而不是替用户编需求。**注意四段不省**：小模块整份可以更短，「模块内约定」确实没有就写一行「无，全局约定即够」，段本身保留。
