...
| `docs/review/` | 开发设计稿快照（`/spec-dev-doc` 的产出）：把某模块的解读、ADR、SPEC 与登记的附加取材汇总成面向开发者的**自包含 HTML** 架构设计稿；版本化，旧版不覆盖 | 模块 SPEC 定稿后，需要一份照着开工、也能对外发的设计稿时生成；反馈中的新信息由你消化回流产物后重生成 |
...
三环各自独立断点：**每个 skill 的草稿 = 它正式产物文件名的 `.md` 换成 `.draft.md`**（如 `specs/订单.draft.md`），随写随落盘、**进 git**（同一工作树换会话随时能接；提交并推送后，换人、换机器也能接力）；产物转正即删草稿。环与环之间只靠产物文件衔接：ADR 更新了，设计要不要跟进由你决定（重跑 `/spec-design`）；没有 SPEC 就跑 `/spec-issues` 会被提示先设计——**访谈完不想设计、设计完不想拆 issue，都可以停，回来接着跑对应的 skill 就行**。需求决策 ADR 追加式保留全部历史；issue 执行状态只看 GitHub、本地不记进度。架构导读住 `docs/ARCHITECTURE.md`（全局图 + 分层导读 + 横切约定 + 关键决策索引；首轮设计拍板后创建，只在结构或全局约定变化时更新，「为什么」一律住 ADR）；SPEC 里另有本轮模块图；模块稳态详图不单独维护——`/spec-dev-doc` 生成开发设计稿时按产物现画。

模块 SPEC 定稿后，随时 `/spec-dev-doc <模块> [版本号]`——把该模块产物（解读、ADR、SPEC + 登记的附加取材如实现蓝图、里程碑票面）拼成一份面向开发者的自包含 HTML 开发设计稿落 `docs/review/`（版本化快照，旧版不覆盖）：整体架构与部署形态、每条核心流程一张时序图、子模块统一模板成章、扩展点落接缝、当前里程碑装配清单——照着能开工，微信/邮件也直接发。它是三环的**旁路**：只读产物、不产生三环的输入；看稿反馈里的实质新信息由你消化，需求级走 `/spec-interview` 增量轮、设计级走 `/spec-design` 修订轮，回流后重生成。
