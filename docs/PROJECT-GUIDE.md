...
| `docs/prd/` | **外来输入区**：PRD、原型、外部架构设计等上游给的文档都放这（只读，不是事实来源——外部架构会被 `/spec-design 全局` 转译成标准 ARCHITECTURE.md 归档）；`README.md` 是**外来输入登记表**——PRD/需求/架构/设计文档的目录或 URI 逐条登记，skill 与会话按它找上游文档（打不开的线上文档落快照，URI 只当溯源）；子目录 `解读/` 存访谈产出的**人话版 PRD**（`解读/<模块>.md`），随讲随写、随时可看 | 上游文档放根、解读归 `解读/`（由 `/spec-interview` 逐块生成），输入与产出不混放 |
...
6. **图优先于文字**：流程、结构、状态关系一律 Mermaid 图，文字只做图的补充说明。图里写什么、字怎么排，见下节「画图规范」。
...
10. **该细的细，该简的简；不堆名词**：
    ...
    - 常规内容一句概括，不逐项罗列操作——「管理账号」说得清的，不再列邀请、改角色、停用、恢复。第 4 条管的是非列不可的并列项，不是把概括拆散了重新列。
    - 影响理解的分工、条件、交接和例外展开写清。给实现者看的接口签名、字段、失败分支属于该细的，照写不省。
    - 压缩不能压掉意思：谁做、什么时候做、做到哪一步，不许为了短写含糊。

    图里的字同样守这条，放到图上的写法见下节第 2–4 条。

### 画图规范（图里写什么、字怎么排）

**该细的细，该简的简**：读者看完图，要知道每一块做什么、彼此怎样配合。标准是零上下文的读者不回正文，也能把图走通。

**本节只管内容和文字排版。** 图种、排布、走线、组件尺寸、字号字体、配色归画图工具：md 里是 Mermaid 自动排版，设计文档交给 diagram-design。不给它下这些指令，只在看到成品后检查。
...
3. **常规职责概括，关键差异展开**：「管理账号」说得清的，不再列邀请、改角色、停用、恢复。影响理解的分工、条件和交接必须写清，如「authentik 确认身份，GC 签发自己的令牌」「首次登录绑定账号；每次登录清除 authentik 会话」。
4. **压缩不能压掉意思**：删重复说明和不必要的细节，保留职责、触发条件、结果与例外。谁做、什么时候做、做到哪一步，不许为了短写含糊。
5. **框内写职责，箭头写交接**：箭头要看得出谁调用谁、传递什么，必要时标明跳转、返回或异步通知。同一件事只说一处，不在框、箭头和图注里反复解释。
6. **实现细节不进图**：表名、字段、存储位置、内部函数、代码目录放正文（SPEC 里是「怎么做」栏）。接口路径只在说明分流或关键交接时保留。图里的名字写真名，不写「本模块」这种代词。
7. **分号分句，分号后换行**：一个框里有两件事，用分号隔开，分号保留、后面主动换行（Mermaid 标签里写 `；<br>`）。逗号、顿号是句内停顿，不据此换行；长句由渲染按组件宽度自然折行。
8. **有回调、跳转、返回的流程图，写明接力参数**：把串起前后步骤的参数（回跳地址、原始地址、一次性码等）逐个交代清楚。
   - 保存类参数写它存的是什么，用前面步骤的产物来说。
   - 回跳类参数写谁处理完之后，让浏览器（或调用方）带着什么回到哪里。
   - 步骤在图上印带前缀的编号（如 L1–L6），说明里按编号指步骤。箭头上只标参数名，含义在图下统一列一次，一个参数一行。例：

   ```
   next：保存 L2 响应中 Location 的完整地址和参数，也就是 L3 已访问过的 authentik 授权地址；
   redirect_uri：authentik 返回授权码或错误信息时，让浏览器跳回的 GC 地址；
   return_to：GC 处理完登录后，让浏览器带着一次性登录码或错误信息返回的 Console 页面。
   ```

9. **看成品，不合格交回画图工具**：一个方向连排超过五六个节点、成品特别扁长或瘦高，就换向、分组或拆图——Mermaid 改 `LR` ⇄ `TB` 或加 subgraph，diagram-design 交回给它重排。看得到渲染结果的，还要查三样：文字完整、没被截断或遮挡；连线接到组件边缘、不穿过组件；组件大小与字数相称。diagram-design 的图不合格就交回它重调，不手改导出物。
10. **改图只动指定范围**：先对照正文核实职责和关系。只改点名的那几张、那一块。改完看渲染结果，确认没问题再给人预览。
...
| 堆名词、该简不简 | 「角色、SSO、部门」一串名词看不出谁做什么；「管理账号」说得清的又逐项列了邀请、停用、恢复 |
...
| 与依据冲突 | 和 PRD、ADR、其他模块的解读或 SPEC、外部架构文档说的不一样。复审前先扫 `docs/prd/README.md` 登记的取材，命中时给出处——别的文档可能已经有解法 |
...
**SPEC 是模块内容的唯一源**：模块的全部内容只存在于 `specs/<模块>.md`，正文按**章**组织，每章装六栏——这章讲什么 / **图** / 为什么这么定 / 要你拍的 / 缺口 / 怎么做。前五栏给拍板的人，第六栏「怎么做」给实现的人。**图归拍板的人一侧**：看得见结构才谈得上判断设计对不对。「要你拍的」「缺口」两栏没有就整栏不写，不留「无」占位。业务章按业务流程组织，数量与命名由模块自己定，没有固定章节清单；固定的只有「决策与未决登记」（决策三件套 + 验证状态，加未拍板清单，设计文档收尾两章从这里投影）与「分期承载」两章。

模块 SPEC 定稿后，随时 `/spec-dev-doc <模块> [版本号]`——把 SPEC 投影成一份自包含 HTML 设计文档落 `docs/review/`（版本化快照，旧版不覆盖）。它**只读 SPEC 一份输入**，只取前五栏（含图），「怎么做」一个字不取；固定的只有开篇「整体设计」与收尾「已决登记」「未决登记」这三章，中间的章镜像 SPEC；全文只有一个端到端例子、文字 ≤6 屏（图不计）。因为只有一个输入，它没法二次创作——实现细节不进来，也就不需要靠折叠压缩。它是三环的**旁路**：只读 SPEC、不产生三环的输入；看稿反馈里的实质新信息由你消化，需求级走 `/spec-interview` 增量轮、设计级走 `/spec-design` 修订轮，回流后重生成。
