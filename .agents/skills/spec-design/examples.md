...
**注意与 SPEC 的分工**：模块图是**稳态全貌**——全部子组件连同并存的旧路径都画，不标新建/改造（那是 SPEC 本轮图的事）；每段只说「是什么」，「为什么」住 ADR（决策索引指路）。**注意详细程度与取材**：模块图信息量对标全局图——分层展开、边标数据流向，主要取材该模块解读（`docs/prd/解读/<模块>.md`），解读里的每条主要流程都要能在图上走通；本例模块小所以图小，真实模块图通常更满，但节点仍控制在十来个。**注意四段不省**：小模块整份可以更短，「模块内约定」确实没有就写一行「无，全局约定即够」，段本身保留。
