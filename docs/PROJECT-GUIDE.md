...
6. **图优先于文字**：流程、结构、状态关系一律 Mermaid 图，文字只做图的补充说明。图要画得自足：节点与连线的标签带足信息——名字 + 半句职责或动作要点，光杆名词/动词不合格；布局横竖两个方向都用——一个方向连排超过五六个节点就换向、分组或拆图，成品特别扁长或瘦高就是重排信号。
