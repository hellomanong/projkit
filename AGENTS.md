...
- `.agents/skills/spec-{interview,design,issues}/`：需求访谈（产出 `docs/adr/` 追加式决策）→ 方案设计与 SPEC 定稿（`specs/` + `docs/architecture/` 模块架构文档 + ARCHITECTURE.md）→ issue 拆分（GitHub 对账）三环流水线——产物文件衔接、各自草稿断点可续、哪环有问题单独修哪环，**随样板分发到每个新项目**；bootstrap 第 4/5 步按序委托它们
...
- 预建目录刻意留空（`.claude/` 下的 rules/agents/commands/hooks/output-styles/workflows/agent-memory，根下的 specs/、docs/architecture/、docs/prd/）——它们是样板结构的一部分，不要往里塞内容来「示范」
