...
**SPEC 是模块内容的唯一源**：模块的全部内容只存在于 `specs/<模块>.md`，正文按**章**组织，每章装六栏——这章讲什么 / **图** / 为什么这么定 / 要你拍的 / 缺口 / 怎么做。前五栏给拍板的人，第六栏「怎么做」给实现的人。**图归拍板的人一侧**：看得见结构才谈得上判断设计对不对。「要你拍的」「缺口」两栏没有就整栏不写，不留「无」占位。业务章按业务流程组织，数量与命名由模块自己定，没有固定章节清单；固定的只有「决策与未决登记」（决策三件套 + 未拍板清单，设计文档收尾两章从这里投影）与「分期承载」两章。
